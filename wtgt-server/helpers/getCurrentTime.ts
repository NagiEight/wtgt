...
const getCurrentTime = (): string => {
    const now: Date = new Date(),
        ...
        formatted: string = `${monthNames[month].slice(0, 3)} ${formatOridinal(day)} ${year} ${(hours <= 12 ? hours : hours - 12).toString().padStart(2)}:${minutes.toString().padStart(2)}:${seconds.toString().padStart(2)} ${hours < 12 ? "a.m" : "p.m"}`;
    return formatted;
};
